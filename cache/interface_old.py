...
# Import ATOMCorntext
from corntext import ATOMCorntext, GROQ_AVAILABLE, CREW_AVAILABLE
...
# --- CORTEX ---
def get_cortex():
    if "corntext_instance" not in st.session_state:
        try:
            st.session_state.corntext_instance = ATOMCorntext()
        except Exception as e:
            st.session_state.corntext_instance = None
            st.error(f"Corntext init error: {e}")
    return st.session_state.corntext_instance
...
with st.sidebar:
    ...
    if corntext:
        ...
        st.error("ERROR")
        if st.button("REINIT"):
            if "corntext_instance" in st.session_state:
                del st.session_state.corntext_instance
            st.rerun()
    
    st.markdown("---")
    st.markdown("### NAVIGATION")
    st.page_link("interface.py", label="💬 Chat")
...
# --- MAIN ---
st.markdown("## A.T.O.M")
st.caption("AUTONOMOUS TASK ORCHESTRATION MACHINE · v3.1 CORTEX")

# Display Chat
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        # Show thinking process if available (expandable)
        if msg.get("thinking"):
